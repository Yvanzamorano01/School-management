...
const ClassRevenueTable = ({ classData, currency }) => {
  return (
          ...
          <tbody>
            {classData?.map((item, index) => (
              <tr key={index} className="border-b border-border last:border-0 hover:bg-muted/50 transition-smooth">
                ...
                <td className="py-3 px-2 md:px-4 text-sm md:text-base text-right font-semibold text-success whitespace-nowrap">
                  {formatCurrency(item?.collected, currency)}
                </td>
                <td className="py-3 px-2 md:px-4 text-sm md:text-base text-right font-semibold text-error whitespace-nowrap">
                  {formatCurrency(item?.outstanding, currency)}
                </td>
                <td className="py-3 px-2 md:px-4 text-right">
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${item?.collectionRate >= 80 ? 'bg-success/10 text-success' :
